...
1. `LOOP1_DEMO_PLAYBOOK.md`：先選 audience route、時間與 appendix。
2. `LOOP1_INVESTOR_DEMO_REHEARSAL.md`：第一次演練、逐步點擊、Investor
   講稿、keywords、Q&A、故障備案與評分。
3. `LOOP1_HANDOFF_AND_TEST.md`：安裝、preflight、驗收。
4. `LOOP1_CUSTOMER_DEMO_10MIN.md`：客戶／工業 domain 版逐分鐘 talk track。
5. `LOOP1_DEMO_MECHANICS_AND_PRODUCT_EVOLUTION.md`：tick、controls、Cases、
   Agent／LLM 邊界與產品演進。
6. `LOOP1_DATA_HUB_AGENT_FLOW.md`：Data Hub 功能與 Agent flow。
7. `LOOP1_VALUE_VALIDATION.md`：capability proof、customer KPI 與 follow-up。
8. `SOLERA_CUSTOMER_USE_CASES.md`：依產業、buyer 與成本選擇場景。
9. `LOOP1_DEMO_SOP.md`：完整技術操作與 troubleshooting。
...
2. **Run Hero scenario**：replay 到已知的 cooling-valve stiction Case，並呼叫
   LOOP-1 backend Agent。
3. **Timeline**：展示 18 個 raw alarms、時間順序與 event cluster。完整的
   command → position → flow → thermal signal lanes 尚未直接畫在 Timeline。
4. **Investigation**：展示 Top-1、Top-3、supporting/counter Evidence 與
   bounded confidence。
5. **Evidence**：展示 SOP Revision 4、P&ID、MOC、maintenance history、
   shift log 與 similar cases。
6. **Request approval**：Core viewer flow 只指出按鈕；Approval appendix 才
   使用 engineer token 建立 draft-only inspection action。兩者都不會寫入
...
使用 Solera：
18 個 raw alarms
→ alarm timeline + causal investigation summary
...
6. 完成後切換到 **Evidence** 檢查 source ID、value、文件、相似案例與 Skill
   trace；完整 lineage details 需由 API payload 檢查。
