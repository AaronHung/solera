...
.loop1-shell {
  ...
  height: 100%;
  display: grid;
  grid-template: 72px minmax(0, 1fr) / 88px minmax(0, 1fr) 250px;
  overflow: hidden;
  color: var(--text);
  background:
    ...
    var(--bg);
  font: 400 16px/1.52 Inter, ui-sans-serif, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
}
.loop1-loading {
  display: flex;
  gap: 16px;
...
.loop1-brand strong { font-size: 19px; letter-spacing: .02em; }
.loop1-brand small, .loop1-pulse small { color: var(--muted); font-size: 14px; }
.loop1-mark {
  width: 38px;
  height: 38px;
  display: grid;
...
.loop1-locale small { font-size: 13px; font-weight: 800; }
.loop1-state { display: inline-flex; align-items: center; gap: 6px; color: var(--cyan); text-transform: uppercase; font-size: 14px; font-weight: 700; }
...
.loop1-hero h1 { margin: 0; font-size: clamp(28px, 2.4vw, 38px); line-height: 1.12; letter-spacing: -.025em; }
.loop1-hero > div > span { display: block; max-width: 760px; margin-top: 8px; color: var(--muted); font-size: 17px; }
.loop1-pulse {
  flex: 0 0 auto;
  gap: 10px;
  min-width: 210px;
...
.loop1-pulse > span { margin-left: auto; color: var(--muted); font-size: 14px; }
.loop1-pulse strong { color: var(--green); text-transform: uppercase; font-size: 15px; }
.loop1-error {
  display: flex;
  gap: 10px;
  align-items: center;
...
.loop1-kpis small { display: block; color: var(--muted); font-size: 14px; letter-spacing: .08em; }
.loop1-kpis strong { display: block; margin: 7px 0 4px; font-size: 30px; font-variant-numeric: tabular-nums; }
.loop1-kpis span { color: var(--muted); font-size: 15px; }
...
.loop1-asset-head strong { font-size: 20px; }
.loop1-asset-head span { color: var(--muted); font-size: 15px; }
...
.loop1-process dt { min-width: 0; display: grid; color: var(--muted); font-size: 15px; }
.loop1-process dt span { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.loop1-process dt code { overflow: hidden; color: var(--dim); font-size: 12px; text-overflow: ellipsis; white-space: nowrap; }
.loop1-process dd { margin: 0; font-size: 16px; font-weight: 700; font-variant-numeric: tabular-nums; }
...
.loop1-process footer small { font-size: 14px; text-transform: uppercase; }
.loop1-action-rail {
  grid-row: 2;
  grid-column: 3;
  display: flex;
...
.loop1-action-rail small { color: var(--muted); font-size: 14px; letter-spacing: .1em; }
...
.loop1-action-message { color: var(--muted); font-size: 14px; }
.loop1-empty {
  min-height: 420px;
  display: grid;
...
.loop1-timeline > header small { color: var(--muted); font-size: 14px; }
.loop1-timeline > header strong { font-size: 30px; }
...
.loop1-timeline-dot { z-index: 1; width: 25px; height: 25px; display: grid; place-items: center; border: 1px solid var(--border); border-radius: 50%; color: var(--muted); background: var(--panel); font-size: 12px; }
...
.loop1-timeline li small, .loop1-timeline li span { color: var(--muted); font-size: 15px; }
.loop1-timeline li strong { margin: 2px 0; font-size: 18px; }
...
.loop1-console > header small { color: var(--amber); font-size: 14px; font-weight: 800; letter-spacing: .12em; }
.loop1-console > header > span { color: var(--dim); font-size: 13px; letter-spacing: .08em; }
.loop1-console-grid { display: grid; grid-template-columns: 1fr; gap: 11px; }
.loop1-console-grid label { min-width: 0; display: grid; align-content: start; gap: 6px; color: var(--muted); }
.loop1-console-grid label > span { color: var(--text); font-size: 15px; font-weight: 700; }
...
.loop1-console-grid label > small { min-height: 44px; color: var(--dim); font-size: 14px; }
...
.loop1-plan ol { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 9px; margin: 10px 0 0; padding: 0; list-style: none; }
.loop1-plan li { display: grid; align-content: start; gap: 5px; color: var(--muted); font-size: 14px; }
...
.loop1-trace > header small { max-width: 310px; color: var(--dim); font-size: 14px; text-align: right; }
.loop1-trace > p { color: var(--muted); font-size: 15px; }
...
.loop1-trace li > span { width: 25px; height: 25px; display: grid; place-items: center; border-radius: 50%; color: var(--dim); background: rgba(255,255,255,.04); font-size: 11px; }
.loop1-trace li > div { display: grid; }
.loop1-trace li strong { font-size: 15px; }
.loop1-trace li small { color: var(--dim); font: 13px/1.45 ui-monospace, SFMono-Regular, Menlo, monospace; }
...
.loop1-findings h2 { margin: 7px 0 13px; font-size: 25px; line-height: 1.4; }
.loop1-findings p { color: var(--muted); font-size: 16px; }
...
.loop1-hypotheses h3 { margin: 2px 0 5px; font-size: 20px; }
.loop1-hypotheses p { margin: 0; color: var(--muted); font-size: 16px; }
.loop1-hypotheses small { color: var(--green); font-size: 14px; text-transform: uppercase; }
.loop1-hypotheses article > strong { font-size: 24px; color: var(--cyan); }
.loop1-hypotheses footer { display: flex; gap: 12px; margin-top: 8px; color: var(--dim); font-size: 14px; }
...
.loop1-draft h3 { margin: 2px 0; font-size: 20px; }
.loop1-draft p { margin: 0; color: var(--muted); font-size: 16px; }
...
.loop1-evidence-list h2 { margin: 0; font-size: 22px; }
.loop1-evidence-list > header span { color: var(--muted); font-size: 15px; }
...
.loop1-evidence-list > article > span { padding: 4px 7px; border-radius: 6px; color: var(--cyan); background: rgba(98,214,210,.07); font-size: 13px; text-transform: uppercase; }
.loop1-evidence-list > article > div { display: grid; }
.loop1-evidence-list small { color: var(--muted); font-size: 14px; }
.loop1-evidence-list code { max-width: 220px; overflow: hidden; color: var(--amber); font-size: 14px; text-overflow: ellipsis; white-space: nowrap; }
...
@media (max-width: 1180px) {
  .loop1-shell { grid-template-columns: 78px minmax(0, 1fr) 225px; }
